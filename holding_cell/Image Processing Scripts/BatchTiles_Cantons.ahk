...
;
; Hotkeys:
;   Ctrl+Alt+B  -> Start batch (sequential zooms)
;   Ctrl+Alt+P  -> Pause/Resume (pauses BETWEEN files)
;   Esc         -> Emergency kill (ExitApp)
;
; Adds: Auto-press "Continue" on Photoshop modal error dialogs
; ============================================================
...
; Paths
; ---------------------------
ROOT := "C:\Users\casst\OneDrive\Desktop\ChatGPT map\tiles_gaius_district_cantons"
...
; Dialog watchdog frequency (ms)
DIALOG_WATCHDOG_MS  := 250
...
^!b:: {  ; Ctrl+Alt+B
    global gBatchRunning, gPaused, DIALOG_WATCHDOG_MS
    if gBatchRunning {
    ...
    ; Start dialog watchdog (auto-click "Continue" on PS modal dialogs)
    SetTimer DialogWatchdog, DIALOG_WATCHDOG_MS

    try {
        Main()
    } finally {
        ; Stop watchdog + reset flags
        SetTimer DialogWatchdog, 0
        gBatchRunning := false
        gPaused := false
    }
}
...
; ============================================================
Main() {
    global ROOT, ZOOMS, PROCESSED_LOG, PHOTOSHOP_EXE, BETWEEN_FILES_MS
    ...
    for _, z in ZOOMS {
        ...
        for _, path in q {
            ...
            ; If an error dialog pops, watchdog will hit "Continue"
            RunPhotoshopAction()
...
WaitIfPaused() {
    ...
    while gPaused {
        Sleep 150
        ToolTip "PAUSED — Press Ctrl+Alt+P to resume"
        ; Let watchdog keep running while paused (it is on a timer)
    }
}
...
OpenFileInPhotoshop(fullPath) {
    ...
    ; IMPORTANT: Don't interfere with this dialog; watchdog only clicks if it sees "Continue"
...
; ============================================================
; Dialog watchdog: auto-click "Continue" on Photoshop modal dialogs
; - Looks for any active #32770 dialog from Photoshop.exe
; - If it has a button whose text contains "Continue", clicks it
; ============================================================
DialogWatchdog() {
    ; Find any active dialog belonging to Photoshop
    hwnd := WinExist("ahk_class #32770 ahk_exe Photoshop.exe")
    if !hwnd
        return

    ; Look for a "Continue" button among Button1..Button6
    Loop 6 {
        btn := "Button" A_Index
        try {
            t := ControlGetText(btn, "ahk_id " hwnd)
        } catch {
            continue
        }
        if InStr(t, "Continue") {
            ; Click the Continue button
            try ControlClick(btn, "ahk_id " hwnd)
            return
        }
    }
}
